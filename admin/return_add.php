<?php
	...
	if(isset($_POST['add'])){
		...
		$sql = "SELECT * FROM users WHERE user_id = '$student'";
			...
			foreach($_POST['isbn'] as $isbn){
				if(!empty($isbn)){
					...
					if($query->num_rows > 0){
						...
						$sql = "SELECT * FROM borrow WHERE user_id = '$student_id' AND book_id = '$bid' AND status = 0";
						$query = $conn->query($sql);
						if($query->num_rows > 0){
							$borrow = $query->fetch_assoc();
							$borrow_id = $borrow['id'];
							$sql = "INSERT INTO returns (user_id, book_id, date_return) VALUES ('$student_id', '$bid', NOW())";
							if($conn->query($sql)){
								$return++;
