...
<?php
	...
	$stuid = $student['id'];
	$sql = "SELECT * FROM borrow LEFT JOIN books ON books.id=borrow.book_id WHERE user_id = '$stuid' ORDER BY date_borrow DESC";
	$action = '';
	if(isset($_GET['action'])){
		$sql = "SELECT * FROM returns LEFT JOIN books ON books.id=returns.book_id WHERE user_id = '$stuid' ORDER BY date_return DESC";
		$action = $_GET['action'];
	}
